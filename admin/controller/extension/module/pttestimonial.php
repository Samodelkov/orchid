...
class ControllerExtensionModulePttestimonial extends Controller
{
    ...
    public function index() {
        $this->load->language('extension/module/pttestimonial');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_setting_module->addModule('pttestimonial', $this->request->post);
            } else {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('extension/module/pttestimonial', 'user_token=' . $this->session->data['user_token'], true)
        );

        $data['action'] = $this->url->link('extension/module/pttestimonial', 'user_token=' . $this->session->data['user_token'], true);
        $data['testimonial_link'] = $this->url->link('tmarket/testimonial', 'user_token=' . $this->session->data['user_token'], true);
        $data['cancel'] = $this->url->link('tmarket/module', 'user_token=' . $this->session->data['user_token'], true);

        if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $module_info = $this->model_setting_module->getModule($this->request->get['module_id']);
            $data['action'] = $this->url->link('extension/module/pttestimonial', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
        }
        ...
        $this->response->setOutput($this->load->view('tmarket/module/pttestimonial', $data));
    }

    private function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/pttestimonial')) {
    ...
    public function install() {
        $this->load->model('tmarket/testimonial');
        $this->model_tmarket_testimonial->install();
    }

    public function uninstall() {
        $this->load->model('tmarket/testimonial');
        $this->model_tmarket_testimonial->uninstall();
    }
}
