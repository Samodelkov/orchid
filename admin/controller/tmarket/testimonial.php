<?php
class ControllerTmarketTestimonial extends Controller
{
    private $error = array();

    public function index() {
        $this->load->language('tmarket/testimonial');
        $this->load->language('tmarket/adminmenu');

        $this->load->model('tmarket/testimonial');
    ...
    public function insert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_tmarket_testimonial->addTestimonial($this->request->post);
            ...
            $this->response->redirect($this->url->link('tmarket/testimonial', 'user_token=' . $this->session->data['user_token'] . $url, true));
        }
    ...
    public function update() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_tmarket_testimonial->editTestimonial($this->request->get['testimonial_id'], $this->request->post);
            $this->session->data['success'] = $this->language->get('text_success');
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $testimonial_id) {
                $this->model_tmarket_testimonial->deleteTestimonial($testimonial_id);
            }
    ...
    protected function getList() {
        ...
        $testimonial_total = $this->model_tmarket_testimonial->getTotalTestimonials($data);

        $results = $this->model_tmarket_testimonial->getTestimonials($data);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('tmarket/testimonial', 'user_token=' . $this->session->data['user_token'], true),
            'separator' => ' / '
        );

        $data['insert'] = $this->url->link('tmarket/testimonial/insert', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['delete'] = $this->url->link('tmarket/testimonial/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);

        foreach ($results as $result) {
            $action = array();
            $action[] = array(
                'text' => $this->language->get('text_edit'),
                'href' => $this->url->link('tmarket/testimonial/update', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $result['pttestimonial_id'], true)
            );
            $data['testimonials'][] = array(
                ...
                'url'               => $this->url->link('tmarket/testimonial/update', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $result['pttestimonial_id'], true)
            );
        }
        ...
        $pagination->url = $this->url->link('tmarket/testimonial', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('tmarket/testimonial/list', $data));
    }
    ...
    private function getForm() {
        ...
        $data['breadcrumbs'][] = array(
            'text'      => $this->language->get('heading_title'),
            'href'      => $this->url->link('tmarket/testimonial', 'user_token=' . $this->session->data['user_token'] . $url, true),
            'separator' => ' / '
        );


        if (!isset($this->request->get['testimonial_id'])) {
            $data['action'] = $this->url->link('tmarket/testimonial/insert', 'user_token=' . $this->session->data['user_token'], true);
        } else {
            $data['action'] = $this->url->link('tmarket/testimonial/update', 'user_token=' . $this->session->data['user_token'] . '&testimonial_id=' . $this->request->get['testimonial_id'], true);
        }

        $data['cancel'] = $this->url->link('tmarket/testimonial', 'user_token=' . $this->session->data['user_token'], true);

        $data['user_token'] = $this->session->data['user_token'];

        if (isset($this->request->get['testimonial_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $testimonial_info = $this->model_tmarket_testimonial->getTestimonial($this->request->get['testimonial_id']);
        }
        ...
        if (isset($this->request->post['testimonial_description'])) {
            ...
            $data['testimonial_description'] = $this->model_tmarket_testimonial->getTestimonialDescriptions($this->request->get['testimonial_id']);
        } else {
        ...
        $this->response->setOutput($this->load->view('tmarket/testimonial/form', $data));
    }

    private function validateForm() {
        if (!$this->user->hasPermission('modify', 'tmarket/testimonial')) {
