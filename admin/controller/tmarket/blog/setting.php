<?php
class ControllerTmarketBlogSetting extends Controller
{
    private $error = array();

    public function index() {
        $this->load->language('tmarket/blog/setting');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->model_tmarket_blog->addBlogSeoUrl($blog_seo_url, $stores);

            $this->session->data['success'] = $this->language->get('text_success');

            $this->response->redirect($this->url->link('tmarket/blog/setting', 'user_token=' . $this->session->data['user_token'], true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('tmarket/blog/setting', 'user_token=' . $this->session->data['user_token'], true)
        );

        $data['action'] = $this->url->link('tmarket/blog/setting', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('tmarket/blog/setting', $data));
    }

    public function validate() {
        if (!$this->user->hasPermission('modify', 'tmarket/blog/setting')) {
