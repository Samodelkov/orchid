<?php
class ControllerTmarketBlogList extends Controller
{
    private $error = array();

    public function index() {
        $this->load->language('tmarket/blog/list');
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $post_list_id = $this->model_tmarket_blog->addPostList($this->request->post);
            $this->model_tmarket_blog->addPostToList($post_list_id, $this->request->post['post']);
            ...
            $this->response->redirect($this->url->link('tmarket/blog/list', 'user_token=' . $this->session->data['user_token'] . $url, true));
        }
    ...
    public function edit() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_tmarket_blog->editPostList($this->request->get['post_list_id'], $this->request->post);
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected'])) {
            foreach ($this->request->post['selected'] as $post_list_id) {
                $this->model_tmarket_blog->deletePostList($post_list_id);
            }
    ...
    public function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('tmarket/blog/list', 'user_token=' . $this->session->data['user_token'] . $url, true)
        );

        $data['add'] = $this->url->link('tmarket/blog/list/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['copy'] = $this->url->link('tmarket/blog/list/copy', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['delete'] = $this->url->link('tmarket/blog/list/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $post_list_total = $this->model_tmarket_blog->getTotalPostList();

        $results = $this->model_tmarket_blog->getAllPostList($filter_data);
        ...
        foreach ($results as $result) {
            $data['post_list'][] = array(
                ...
                'edit'          => $this->url->link('tmarket/blog/list/edit', 'user_token=' . $this->session->data['user_token'] . '&post_list_id=' . $result['post_list_id'] . $url, true)
            );
        }
        ...
        $pagination->url = $this->url->link('tmarket/blog/list', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('tmarket/blog/list/list', $data));
    }
    ...
    public function getForm() {
        ...
        if (!isset($this->request->get['post_list_id'])) {
            $data['action'] = $this->url->link('tmarket/blog/list/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
        } else {
            $data['action'] = $this->url->link('tmarket/blog/list/edit', 'user_token=' . $this->session->data['user_token'] . '&post_list_id=' . $this->request->get['post_list_id'] . $url, true);
        }

        $data['cancel'] = $this->url->link('tmarket/blog/list', 'user_token=' . $this->session->data['user_token'] . $url, true);

        if (isset($this->request->get['post_list_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $post_list_info = $this->model_tmarket_blog->getPostList($this->request->get['post_list_id']);
            $post_list_info['post'] = $this->model_tmarket_blog->getPostToList($this->request->get['post_list_id']);
        }
        ...
        if (isset($this->request->post['post_list_description'])) {
            ...
            $data['post_list_description'] = $this->model_tmarket_blog->getPostListDescriptions($this->request->get['post_list_id']);
        } else {
        ...
        if (isset($this->request->post['post_list_store'])) {
            ...
            $data['post_list_store'] = $this->model_tmarket_blog->getPostListStores($this->request->get['post_list_id']);
        } else {
        ...
        foreach ($posts as $post) {
            $post_info = $this->model_tmarket_blog->getPost($post['post_id']);
        ...
        $this->response->setOutput($this->load->view('tmarket/blog/list/form', $data));
    }

    protected function validateForm() {
        if (!$this->user->hasPermission('modify', 'tmarket/blog/list')) {
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['filter_name']) ) {
            ...
            $results = $this->model_tmarket_blog->getPostLists($filter_data);
