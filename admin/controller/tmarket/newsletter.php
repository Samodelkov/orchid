<?php
class ControllerTmarketNewsletter extends Controller
{
    public function index() {
        $this->load->language('tmarket/newsletter');
        $this->load->language('tmarket/adminmenu');

        $this->document->setTitle($this->language->get('page_title'));

        $this->load->model('tmarket/newsletter');
    ...
    public function editSubscribe() {
        ...
        if(isset($this->request->get['mail_id']) && isset($this->request->get['subscribe'])) {
            ...
            $this->model_tmarket_newsletter->editSubscribe($mail_id, $subscribe_status);
            ...
            $this->response->redirect($this->url->link('tmarket/newsletter', 'user_token=' . $this->session->data['user_token'] . $url, true));
        }
    ...
    public function delete() {
        ...
        if(($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $mail_id) {
                $this->model_tmarket_newsletter->deleteMail($mail_id);
            }
    ...
    public function getList() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('tmarket/newsletter', 'user_token=' . $this->session->data['user_token'] . $url, true)
        );

        $data['delete'] = $this->url->link('tmarket/newsletter/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
        $data['send_form'] = $this->url->link('tmarket/newsletter/sendForm', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $mails_total = $this->model_tmarket_newsletter->getTotalMails($filter_data);

        $results = $this->model_tmarket_newsletter->getMails($filter_data);
        ...
        foreach ($results as $result) {
            ...
            $data['mails'][] = array(
                ...
                'edit' => $this->url->link('tmarket/newsletter/editSubscribe', 'user_token=' . $this->session->data['user_token'] . '&mail_id=' . $result['newsletter_id'] . '&subscribe=' . $changeStatus . $url, true)
            );
        }
        ...
        $pagination->url = $this->url->link('tmarket/newsletter', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('tmarket/newsletter/list', $data));
    }
    ...
    public function sendForm() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('tmarket/newsletter/sendForm', 'user_token=' . $this->session->data['user_token'] . $url, true)
        );

        $data['cancel'] = $this->url->link('tmarket/newsletter', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $this->response->setOutput($this->load->view('tmarket/newsletter/mail', $data));
    }
    ...
    public function sendMail() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            if (!$this->user->hasPermission('modify', 'tmarket/newsletter')) {
            ...
            if (!$json) {
                ...
                switch ($this->request->post['to']) {
                    case 'all':
                        ...
                        $email_total = $this->model_tmarket_newsletter->getTotalMails($filter_data);
                    ...
                    case 'specified':
                        ...
                        foreach ($subscribers as $subscriber) {
                            $newsletter = $this->model_tmarket_newsletter->getMail($subscriber);
                ...
                if ($emails) {
                    ...
                    if ($end < $email_total) {
                        $json['next'] = str_replace('&amp;', '&', $this->url->link('tmarket/newsletter/sendMail', 'user_token=' . $this->session->data['user_token'] . '&page=' . ($page + 1), true));
                    } else {
                        $json['next'] = '';
                    }
