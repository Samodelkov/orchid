...
var ptmenu = {
    ...
    'submitTopItemForm' : function (menu_id) {
        ...
        $.ajax({
            ...
            success : function (json) {
                if(json['submit'] === true) {
                    ptmenu.getItemsToContainer(menu_id);
                } else {
                    flag = false;
                    $('#form-container-bg').show();
                    $('#form-ajax-loader').show();
            ...
            success     : function (json) {
                ...
                if(top_items) {
                    ...
                    for(i in top_items) {
                        ...
                        html += '           <a href="javascript:void(0)" title="' + text_add_sub_item + '" onclick="ptmenu.appendAddSubItemForm(\'' + top_items[i]['menu_item_id'] + '\', \'2\')" class="a-config"><i class="fa fa-plus"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="ptmenu.appendTopItemNewForm(\'' + top_items[i]['url'] + '\')" class="a-config"><i class="fa fa-pencil"></i></a>';
                        html += '           <a href="javascript:void(0)" onclick="ptmenu.deleteMenuItem(\'' + top_items[i]['del_url'] + '\')" class="a-delete a-config"><i class="fa fa-times"></i></a>';
                        html += '       </div>';
                        html += '   </div>';
                        ...
                        if(top_items[i]['sub_items']) {
                            ...
                            for(j in sub_items) {
                                ...
                                html += '       </div>';
                                html += '       <div class="action">';
                                html += '           <a href="javascript:void(0)" title="' + text_add_sub_item + '" onclick="ptmenu.appendAddSubItemForm(\'' + sub_items[j]['item_id'] + '\', \'3\')" class="a-config"><i class="fa fa-plus"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="ptmenu.appendEditSubItemForm(\'' + sub_items[j]['item_id'] + '\')" class="a-config"><i class="fa fa-pencil"></i></a>';
                                html += '           <a href="javascript:void(0)" onclick="ptmenu.deleteMenuItem(\'' + sub_items[j]['del_url'] + '\')" class="a-delete a-config"><i class="fa fa-times"></i></a>';
                                html += '       </div>';
                                html += '   </div>';
                                ...
                                if(sub_items[j]['sub_items']) {
                                    ...
                                    for(k in s_items) {
                                        ...
                                        html += '       </div>';
                                        html += '       <div class="action">';
                                        html += '           <a href="javascript:void(0)" onclick="ptmenu.appendEditSubItemForm(\'' + s_items[k]['item_id'] + '\')" class="a-config"><i class="fa fa-pencil"></i></a>';
                                        html += '           <a href="javascript:void(0)" onclick="ptmenu.deleteMenuItem(\'' + s_items[k]['del_url'] + '\')" class="a-delete a-config"><i class="fa fa-times"></i></a>';
                                        html += '       </div>';
                                        html += '   </div>';
            ...
            success : function (json) {
                ...
                if(child_categories) {
                    for(i in child_categories) {
                        ...
                        html += '       <a class="a-resize-minus a-left" onclick="ptmenu.decreaseColumn($(this))"><i class="fa fa-chevron-left" aria-hidden="true"></i></a>';
                        html += '       <a class="a-resize-plus a-left" onclick="ptmenu.increaseColumn($(this))"><i class="fa fa-chevron-right" aria-hidden="true"></i></a>';
                        html += '       <div class="widget-name-container">';
                        html += '           <span class="name">' + child_categories[i]['name'] + '</span>';
                        html += '       </div>';
                        html += '       <div class="widget-action">';
                        html += '           <a class="a-config" onclick="ptmenu.showWidgetConfiguration($(this))"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                        html += '           <a class="a-delete a-config" onclick="ptmenu.deleteWidget($(this))"><i class="fa fa-times" aria-hidden="true"></i></a>';
                        html += '       </div>';
                        html += '   </div>';
    ...
    'addWidget': function() {
        ...
        if(widget_type == 'html') {
            ...
            $.ajax({
                url : 'index.php?route=tmarket/ultimatemenu/getLanguageData&user_token=' + user_token,
                type: 'get',
                success : function (json) {
    ...
    'chooseCategory' : function () {
        ...
        $('.w-category').customAutoComplete({
            'source': function(request, response) {
                $.ajax({
                    url: 'index.php?route=tmarket/ultimatemenu/autoCompleteCategory&user_token=' + user_token + '&filter_name=' +  encodeURIComponent(request),
                    dataType: 'json',
                    success: function(json) {
