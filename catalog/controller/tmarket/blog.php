<?php
class ControllerTmarketBlog extends Controller
{
    public function index() {
        $this->load->language('tmarket/blog');

        $this->load->model('tmarket/blog');
        ...
        $this->document->addLink($this->url->link('tmarket/blog'), '');
        ...
        $data['list_content_link'] = $this->url->link('tmarket/blog', 'content=list' . $url, true);
        $data['grid_content_link'] = $this->url->link('tmarket/blog', 'content=grid' . $url, true);
        ...
        $post_total = $this->model_tmarket_blog->getTotalPosts($filter_data);

        $results = $this->model_tmarket_blog->getPosts($filter_data);
        ...
        foreach ($results as $result) {
            ...
            $data['posts'][] = array(
                ...
                'href'        => $this->url->link('tmarket/blog/post', '&post_id=' . $result['post_id'] . $url)
            );
        }
        ...
        foreach($limits as $value) {
            $data['limits'][] = array(
                'text'  => $value,
                'value' => $value,
                'href'  => $this->url->link('tmarket/blog', $url . '&limit=' . $value, true)
            );
        }
        ...
        $pagination->url = $this->url->link('tmarket/blog', $url . '&page={page}');
        ...
        $this->response->setOutput($this->load->view('tmarket/blog/list', $data));
    }
    ...
    public function category() {
        ...
        $category_info = $this->model_tmarket_blog->getPostList($post_list_id);
        ...
        if($category_info) {
            ...
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('tmarket/blog', $url, true)
            );

            $this->document->setTitle($category_info['meta_title']);
            $this->document->setDescription($category_info['meta_description']);
            $this->document->setKeywords($category_info['meta_keyword']);
            $this->document->addLink($this->url->link('tmarket/blog/category', 'post_list_id=' . $this->request->get['post_list_id'], true), true);
            ...
            $data['list_content_link'] = $this->url->link('tmarket/blog/category', 'post_list_id=' . $this->request->get['post_list_id'] . '&content=list' . $url, true);
            $data['grid_content_link'] = $this->url->link('tmarket/blog/category', 'post_list_id=' . $this->request->get['post_list_id'] . '&content=grid' . $url, true);
            ...
            foreach($limits as $value) {
                $data['limits'][] = array(
                    'text'  => $value,
                    'value' => $value,
                    'href'  => $this->url->link('tmarket/blog/category', 'post_list_id=' . $this->request->get['post_list_id'] . $url . '&limit=' . $value, true)
                );
            }
            ...
            $results = $this->model_tmarket_blog->getPostsByList($filter_data, $post_list_id);
            ...
            $pagination->page = $page;
            $pagination->limit = $limit;
            $pagination->url = $this->url->link('tmarket/blog/category', 'post_list_id=' . $this->request->get['post_list_id'] . $url . '&page={page}');
            ...
            $this->response->setOutput($this->load->view('tmarket/blog/category', $data));
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('text_error'),
                'href' => $this->url->link('tmarket/blog/category', '&post_id=' . $post_list_id)
            );
    ...
    public function post() {
        ...
        $post_info = $this->model_tmarket_blog->getPost($post_id);
        ...
        if ($post_info) {
            ...
            $this->document->addLink($this->url->link('tmarket/blog/post', 'post_id=' . $this->request->get['post_id'], true), true);
            ...
            if($show_related) {
                $related_posts = $this->model_tmarket_blog->getRelatedPosts($post_id);
                ...
                if($related_posts) {
                    ...
                    foreach ($related_posts as $pid) {
                        $related_post_info = $this->model_tmarket_blog->getPost($pid);

                        $image = $this->model_tool_image->resize($related_post_info['image'], 75, 50);
                        $image_full = $this->model_tool_image->resize($related_post_info['image'], $image_size_width, $image_size_height);
                        ...
                        $data['related_posts'][] = array(
                            ...
                            'href'        => $this->url->link('tmarket/blog/post', '&post_id=' . $related_post_info['post_id'] . $url)
                        );
                    }
                }
            }
            ...
            $this->response->setOutput($this->load->view('tmarket/blog/post', $data));
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('text_error'),
                'href' => $this->url->link('tmarket/blog/post', '&post_id=' . $post_id)
            );
    ...
    public function categories_list() {
            ...
            if(!empty($this->config->get('module_ptblog_cates_list'))) {
                ...
                if($cate_list_ids) {
                    ...
                    foreach ($cate_list_ids as $cate_id) {
                        $cate_info = $this->model_tmarket_blog->getPostList($cate_id);

                        if($cate_info) {
                            $data['categories'][] = array(
                                'name'  => $cate_info['name'],
                                'href'  => $this->url->link('tmarket/blog/category', '&post_list_id=' . $cate_id . $url, true)
                            );
                        }
                    }
                }
        ...
        return $this->load->view('tmarket/blog/widget/cate_list', $data);
    }
    ...
    public function latest_blog($latest_blog_show, $limit) {
        ...
        if($latest_blog_show) {
            ...
            $results = $this->model_tmarket_blog->getPosts($filter_data);
            foreach ($results as $result) {
                $image = $this->model_tool_image->resize($result['image'], 75, 50);
        ...
        return $this->load->view('tmarket/blog/widget/latest_blog', $data);
    }
}
