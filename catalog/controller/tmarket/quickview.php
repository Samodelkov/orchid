<?php
class ControllerTmarketQuickview extends Controller
{
    public function index() {
        ...
        if(!$json) {
            if($data) {
                $json['html'] = $this->load->view('tmarket/quickview/product', $data);
                $json['success'] = true;
            } else {
                $json['success'] = false;
    ...
    public function seoview() {
        ...
        $this->load->model('tmarket/quickview');

        if(!$json) {
            if (isset($this->request->get['ourl'])) {
                $seo_url = $this->request->get['ourl'];

                $product_id = $this->model_tmarket_quickview->getProductBySeoUrl($seo_url);
    ...
    public function container() {
        $this->load->language('tmarket/quickview');
        ...
        return $this->load->view('tmarket/quickview/qvcontainer', $data);
    }
