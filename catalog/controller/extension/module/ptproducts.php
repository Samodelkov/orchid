...
class ControllerExtensionModulePtproducts extends Controller
{
    public function index($setting) {
        $this->load->language('tmarket/module/ptproducts');

        $this->load->model('catalog/product');
        $this->load->model('tmarket/catalog');
            ...
            if($single_category_product_collection == 'special') {
                ...
                if($single_category_product_special_type) {
                    ...
                    if($single_category_product_special_type == 'mostviewed') {
                        $results = $this->model_tmarket_catalog->getMostViewedByCategory($limit, $single_category_id);
                    }

                    if($single_category_product_special_type == 'bestseller') {
                        $results = $this->model_tmarket_catalog->getBestSellerProductsByCategory($limit, $single_category_id);
                    }

                    if($single_category_product_special_type == 'special') {
                        $results = $this->model_tmarket_catalog->getProductSpecialsByCategory($limit, $single_category_id);
                    }

                    if($single_category_product_special_type == 'latest') {
                        $results = $this->model_tmarket_catalog->getLatestProductsByCategory($limit, $single_category_id);
                    }

                    if($single_category_product_special_type == 'random') {
                        $results = $this->model_tmarket_catalog->getRandomByCategory($limit, $single_category_id);
                    }
        ...
        if($single_collection_type == 'special') {
            ...
            if($single_product_special_type) {
                ...
                if($single_product_special_type == 'mostviewed') {
                    $results = $this->model_tmarket_catalog->getMostViewed($limit);
                }
                ...
                if($single_product_special_type == 'random') {
                    $results = $this->model_tmarket_catalog->getRandom($limit);
                }
                    ...
                    if($tab_category_product_type == "special") {
                        ...
                        if($tab_category_product_special_type) {
                            ...
                            if($tab_category_product_special_type == 'mostviewed') {
                                $results = $this->model_tmarket_catalog->getMostViewedByCategory($limit, $tab_category_id);
                            }

                            if($tab_category_product_special_type == 'bestseller') {
                                $results = $this->model_tmarket_catalog->getBestSellerProductsByCategory($limit, $tab_category_id);
                            }

                            if($tab_category_product_special_type == 'special') {
                                $results = $this->model_tmarket_catalog->getProductSpecialsByCategory($limit, $tab_category_id);
                            }

                            if($tab_category_product_special_type == 'latest') {
                                $results = $this->model_tmarket_catalog->getLatestProductsByCategory($limit, $tab_category_id);
                            }

                            if($tab_category_product_special_type == 'random') {
                                $results = $this->model_tmarket_catalog->getRandomByCategory($limit, $tab_category_id);
                            }
        ...
        if($show_countdown) {
            $this->document->addScript('catalog/view/javascript/tmarket/countdown/countdown.js');
        }

        if($show_swatches_image) {
            $this->document->addScript('catalog/view/javascript/tmarket/swatches/swatches.js');
        }

        return $this->load->view('tmarket/module/ptproducts', $data);
    }
    ...
    public function getProductData($product_id, $params, $layout, $section_id) {
        ...
        $result = $this->model_tmarket_catalog->getProduct($product_id);
        ...
        if($result) {
            ...
            if($result['description']) {
                $description = utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0,60, $this->config->get('theme_' . $this->config->get('config_theme') . '_product_description_length')) . '...';
            } else {
                $description = false;
            }
            ...
            if($params['show_hover_image'] == 1) {
                $rotate_image = $this->model_tmarket_rotateimage->getProductRotateImage($product_id);
            ...
            $html_content = $this->load->view('tmarket/module/ptproducts/content', $data);
