...
class ControllerExtensionModulePtmenu extends Controller
{
    public function index($setting) {
        $this->load->language('tmarket/module/ptmenu');
        ...
        $menu = $this->model_tmarket_ultimatemenu->getMenuById($menu_id);

        $data['menu_type'] = $menu['menu_type'];

        if($menu) {
            if($menu['status']) {
                $top_items = $this->model_tmarket_ultimatemenu->getTopItems($menu_id);
                ...
                foreach ($top_items as $top_item) {
                    ...
                    $sub_items2 = $this->model_tmarket_ultimatemenu->getSubItems($top_item['menu_item_id'], '2');

                    foreach ($sub_items2 as $sub_item2) {
                        $sub_items_lv3 = array();

                        $sub_items3 = $this->model_tmarket_ultimatemenu->getSubItems($sub_item2['sub_menu_item_id'], '3');

                        foreach ($sub_items3 as $sub_item3) {
                            $third_title = $this->model_tmarket_ultimatemenu->getSubItemDescriptionById($sub_item3['sub_menu_item_id']);
                        ...
                        $second_title = $this->model_tmarket_ultimatemenu->getSubItemDescriptionById($sub_item2['sub_menu_item_id']);
                    ...
                    $top_item_title = $this->model_tmarket_ultimatemenu->getTopItemDescriptionById($top_item['menu_item_id']);
...
//        echo "<pre>"; var_dump($data['items']);die;
        ...
        $this->document->addScript('catalog/view/javascript/tmarket/ultimatemenu/menu.js');

        return $this->load->view('tmarket/module/ptmenu', $data);
    }
