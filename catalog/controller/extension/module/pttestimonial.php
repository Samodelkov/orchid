...
class ControllerExtensionModulePttestimonial extends Controller
{
    public function index($setting) {
        $this->load->language('tmarket/module/pttestimonial');
        ...
        $testimonial_total = $this->model_tmarket_testimonial->getTotalTestimonials();
        ...
        $results = $this->model_tmarket_testimonial->getTestimonials(0, $limit);
        if($results) {
            foreach($results as $result){
                ...
                $data['testimonials'][] = array(
                    ...
                    'image'             => $timage,
                    'content'   		=> utf8_substr(strip_tags(html_entity_decode($result['content'], ENT_QUOTES, 'UTF-8')), 0, 200) . '..',
                );
            }
        }
        ...
        return $this->load->view('tmarket/module/pttestimonial', $data);
    }
}
